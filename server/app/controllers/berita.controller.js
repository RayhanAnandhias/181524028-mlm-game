...
exports.create = (req, res) => {
    ...
    Berita.create(berita).then(data => {
        res.send(data);
    }).catch(err => {
        res.status(500).send({
...
exports.findAll = (req, res) => {
    ...
    Berita.findAll({ where: condition })
      ...
      .catch(err => {
        res.status(500).send({
          message:
            err.message || "Some error occurred while retrieving berita."
        });
      });
};
...
exports.findAllByCategory = (req, res) => {
    ...
    var condition = kategori ? { kategori: { [Op.iLike]: `%${kategori}%` } } : null;
    Berita.findAll({ where: condition }).then(data => {
        res.send(data);
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Some error occurred while retrieving berita by category."
        })
    })
};